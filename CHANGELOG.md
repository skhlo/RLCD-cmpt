...
- **Non-English (CJK) sessions: token accounting, text primitives, and `recall` queries are now script-aware** ([#106](https://github.com/k0valik/pi-blackhole/issues/106)). Three layers underneath the #105 extractors assumed the user's text is English and broke measurably on a real Chinese-language session: (1) `estimateStringTokens` was `ceil(chars/4)` — an ASCII heuristic that under-counts CJK text ~3× (ideographs/kana/hangul are ~1 token/char in BPE vocabularies) — so every token budget built on it (OM observation/reflection `tokenCount`, pool render caps, `retainedToolOutputMaxTokens`, worker context pre-check) silently admitted ~3× the configured real tokens; it now counts CJK chars as ~1 token each and the remainder at /4, erring safely high for budgets. (2) Sentence/word clipping degenerated to hard character cuts on space-free CJK: `clipSentence` only recognized `[.!?]` followed by whitespace and now also accepts CJK terminators (`。！？；`, fullwidth forms included) followed directly by text; `clip` falls back to the last CJK pause punctuation (`，。！？；、：`) in the window when no usable space boundary exists (English behavior and the surrogate-pair guard unchanged). (3) `recall` split queries on `\s+`, compiling a whole Chinese sentence into one literal pattern that only matched verbatim — CJK query segments are now word-segmented via `Intl.Segmenter` (ICU dictionary, lazily constructed with the same fallback `brief.ts` already used, now extracted to a shared `src/core/segment.ts`), so natural-language Chinese queries rank via per-term BM25 like English ones do; BM25 document length uses a script-aware word count (a 500-char CJK paragraph previously counted as 1 word, voiding length normalization); operator-bearing terms (`login|auth`) and short verbatim substrings behave exactly as before. No new config keys — script detection is automatic; `estimateEntryTokens`'s message path still delegates to Pi's own estimator.

- **`[Files And Changes]` now attributes files instead of guessing them** ([#105](https://github.com/k0valik/pi-blackhole/issues/105)). The old extractor matched a hardcoded tool-name list plus arg sniffing, so on a modern session it named ten already-deleted `/tmp` scratch scripts and none of the six delivered files. New `src/extract/file-touch.ts` (ported from the proven pi-files-touched core) correlates tool calls with results: native tools via path args, anchor-based `replace`/`insert` via result-text recovery, and a bash parser (redirects, `sed -i`, `tee`, `cp`, `mv`, `rm`/`git rm`, `touch`, `patch`, `curl -o`, `cat`/`head`/`tail` reads; heredocs, `/dev/null`, non-literals skipped). Delete-last files are dropped; the collector runs only at compaction time, never per tool call. Pi `fileOps` seeds and the legacy block scan remain as fallback; raw arg strings are sanitized (quotes, trailing punctuation, `:line`/`:start-end`/`#L` suffixes) so `src/a.ts:10-40` no longer phantoms next to `/repo/src/a.ts`. Touch-collector output seeds first (recency-ordered), so each capped line keeps the latest touches. Git grounding adds word tags (`staged`, `unstaged`, `staged,unstaged`, `new`, `renamed`, `deleted`, `conflicted`) from one bounded snapshot (2s/5s timeouts, fail-closed) and promotes untracked-but-written files to `Created:`. Across compactions the merge rejoins wrapped lines, requires line-boundary headers, splits entries on top-level commas only, strips stale prev tags, and parses fresh-first — so the 10-item cap keeps the 10 most recent files instead of stale ones.

- **`[Commits]` now captures the commit forms a careful developer actually uses** ([#105](https://github.com/k0valik/pi-blackhole/issues/105)). Quote-aware token parser replaces the literal `git commit` + single-line `-m` match: any flag order, continuations joined, heredoc bodies stripped from detection. Message recovery chain: segment-scoped `-m`/`--message` (`=` and space forms) → `-F -` heredoc subject (the stdin flag must sit in a segment that invokes `git commit`, so even an earlier command's own `-F -` heredoc can't shadow the message) → git's `[branch hash] subject` success line (incl. root-commit/detached-HEAD) → hash-only placeholder → drop when nothing is recoverable. Precision guards: `--dry-run` (plain and shell-quoted) and the short `-n` alias, failed commits, error results, nonzero exits, and sibling-tool results (name-checked pairing) all produce nothing, and the segment's command word must actually be `git` (so `echo git commit -m …` never records; `sudo`/`env`/`VAR=` prefixes still accepted).

- **`[User Preferences]` no longer assumes English** ([#105](https://github.com/k0valik/pi-blackhole/issues/105)). Correction anchors in English (`stop using …`, `that's wrong`, `revert/undo that`) and CJK (`不要`/`不用`/`别再`/`回退`/`错了`/`停止`, broad negatives like `不行`/`不对` deliberately excluded), CJK floor 2 (bare `回退` fires), future-scoped markers (`以后`/`下次`/`必须`, plus `记住` with an acknowledgement guard). Information questions are rejected but directive questions survive — English via opener gate, CJK via anywhere-match (`为什么以后要用 pnpm？` and `以后怎么提交代码？` both drop), including when the line is wrapped in quotation marks. Rhetorical why-nudges (`Why don't we use pnpm?`, `为什么不用 pnpm？`) drop in both languages by design — telling them apart from information-seeking needs semantic analysis. Marker-less implicit directives stay out by design (no semantic analysis, no false positives).
...
- **`[Files And Changes]` display and merge overhauled**: paths render cwd-relative (outside-cwd ranked last), Modified/Created as indented one-per-line lists up to 20, Read comma-joined up to 10. Cross-compaction merge parses both legacy comma-joined and new counted-list formats, reassembles paths that `wrapLongLines` hard-broke mid-token (a >118-char single-token path previously split into two phantom entries at merge), carries the previous section's total forward so a capped list (`Modified (26):`) never shrinks to the parsed entry count, preserves fresh tags, strips stale ones, dedups by stripped path. Session goal clip raised from 80→200 chars. Outstanding context clip raised 150→200; a pathless same-tool success no longer extinguishes an error that names a path (only two pathless results do). Bash errors extinguished only when the identical command later succeeds; edit/write errors when the same file later succeeds. `[Scope change]` marker now carries its bullets' `(#N)` ref. `[Commits]` lines now emit `(#N)` refs for drill-down via `recall`.
