...
- **Bounded, evaluation-only Jev workflow.** A separate CLI prepares canonical `jev-1.13.0` Noul batches over complete lexical candidate sets, enforces field/request/response/deadline bounds, and supports explicit approved calibration, held-out comparison, and human review evidence. The native no-redirect/no-retry fetch adapter is opt-in; complete unknown-first response validation falls back to the whole lexical order on any failure. Synthetic and scripted results stay visibly blocked and are not quality, budget, cost, reliability, or latency evidence. This does not add runtime reranking, settings, caches, credentials, installed pins, or recall-output changes.
