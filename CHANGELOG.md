...
- **Deterministic offline lexical replay evaluator.** A credential-free CLI runs the existing shared recall query planner, session loader, and BM25+ search over explicit versioned fixtures. Public synthetic tuning and held-out partitions cover reviewed answers, no-answer cases, distractors, corrections, commands, and errors. Reports keep malformed or contradictory truth visible, separate candidate misses from rank misses, average unrounded reciprocal ranks, use known-answer denominators, and include input digests, owner-supplied BM25+ constants, and an exact executable digest for reproducibility. The deliberately constructed fixture metrics test mechanics only.
- **Bounded, evaluation-only Jev workflow.** A separate CLI prepares canonical `jev-1.13.0` Noul batches over complete lexical candidate sets, enforces field/request/response/deadline bounds, and supports explicit approved calibration, held-out comparison, and human review evidence. The standard native no-redirect/no-retry fetch adapter is opt-in; injected transports are scripted evidence, caller cancellation propagates, and complete unknown-first response validation falls back to the whole lexical order on ordinary failures. Frozen preparation and validated report/plan/implementation bindings guard readiness. Provider-reported, estimated, and unknown usage/cost stay distinct, while no-answer/adversarial gates require complete eligible judgments. Synthetic and scripted results stay visibly blocked and are not quality, budget, cost, reliability, or latency evidence. This does not add runtime reranking, settings, caches, credentials, installed pins, or recall-output changes.
